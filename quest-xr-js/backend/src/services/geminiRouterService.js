import { callGeminiGenerateContent, extractTextFromGeminiResponse } from "./geminiRestClient.js";
import { DEFAULT_GEMINI_ROUTER_MODEL } from "../configs/gemini.config.js";
...
export async function classifyIntent(text) {
  ...
  const model = process.env.GEMINI_ROUTER_MODEL || DEFAULT_GEMINI_ROUTER_MODEL;
  ...
  try {
    ...
    console.log(`🤖 Flash-Lite raw output: "${content.trim()}"`);
    ...
    console.error("Error in Cloud Router (Flash-Lite):", error);
    const latencyMs = Math.round(performance.now() - startTime);
    return { categoria: "CONVERSACION_GENERAL", target: "", latencyMs };
  }
}
